			...
			<div class="post-announce">
				...
				<ul class="sub-list">
					...
					<?php if($item[confirm] == 0) {?>
						<li class="err">Не подтверждено</li>
					<?php } ?>
					<?php if($item['is_actual'] == 0) { ?>
						<li class="err">Уже не актуально <a href="/?action=retime_post&id=<?php echo $item['id']; ?>">Продлить на неделю</a></li>
					<?php } ?>
					<li>
						<a href="?action=edit_post&id=<?php echo $item['id']; ?>">Изменить</a>
						<a href="?action=delete_post&id=<?php echo $item['id']; ?>">Удалить</a>
					</li>
				</ul>
			</div>
		</li>
